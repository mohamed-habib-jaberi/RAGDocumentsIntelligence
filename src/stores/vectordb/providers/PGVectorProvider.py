...
class PGVectorProvider(VectorDBInterface):
    ...
    """Implement the PGVector integration behind its application interface."""
    def __init__(self, db_client, default_vector_size: int = 786,
                       distance_method: str = None, index_threshold: int=100):

        """Configure asynchronous vector persistence through PostgreSQL and PGVector."""
    ...
    async def is_index_existed(self, collection_name: str) -> bool:
        """Return whether the vector index for a collection currently exists."""
    ...
    async def create_vector_index(self, collection_name: str,
        ...
        """Create the configured approximate-nearest-neighbor index when required."""
    ...
    async def reset_vector_index(self, collection_name: str, 
        ...
        """Drop and recreate the vector index for the requested collection."""
        index_name = self.default_index_name(collection_name)
        async with self.db_client() as session:
            async with session.begin():
