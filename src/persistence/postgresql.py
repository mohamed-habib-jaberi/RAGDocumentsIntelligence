...
from models.db_schemes.minirag.schemes import Asset, DataChunk, Project
...
class PostgresPersistence:
    """Async SQLAlchemy implementation. Schema changes are managed by Alembic."""
    ...
    async def get_or_create_project(self, project_id):
        ...
        async with self.session_factory() as session:
            ...
            if project is None:
                project = Project(project_id=numeric_id)
                session.add(project)
                await session.commit()
                await session.refresh(project)
            return ProjectRecord(id=project.project_id, project_id=project.project_id)
    ...
    async def list_assets(self, project_id, asset_type):
        async with self.session_factory() as session:
            ...
            return [self._asset(record) for record in result.scalars().all()]
    ...
    async def list_chunks(self, project_id, page, page_size):
        async with self.session_factory() as session:
            ...
            return [self._chunk(record) for record in result.scalars().all()]
